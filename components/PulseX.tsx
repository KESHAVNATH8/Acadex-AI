...
import { User, BookOpen, ChevronRight, RefreshCw, Printer, Sparkles, Users, FileText, Share2, Mail, FileDown, Check } from 'lucide-react';
...
export const PulseX: React.FC = () => {
  ...
  const feedbackShareRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (feedbackShareRef.current && !feedbackShareRef.current.contains(event.target as Node)) {
        setFeedbackShareOpen(false);
      }
    };
  ...
  const formatForPrint = (text: string) => {
    ...
    // Lists
    formatted = formatted.replace(/^\d+\. (.*$)/gim, '<div style="display:flex; margin-bottom: 10px;"><span style="font-weight:bold; margin-right:10px;">•</span><span>$1</span></div>');
        ...
        <html>
          <head>
            ...
            <style>
              ...
              h1 { font-size: 24px; text-transform: uppercase; border-bottom: 2px solid #333; padding-bottom: 10px; }
          ...
          <body>
            <div class="header">
                <h1>${title}</h1>
                <p>Generated by Acadex AI</p>
            </div>
            <div class="content">${formattedContent}</div>
            <div class="footer">Acadex AI • PulseX Feedback System</div>
          </body>
        </html>
      `);
  ...
  const handleEmail = (content: string, subject: string) => {
        ...
        <div className="space-y-6 animate-fade-in">
            ...
            {feedbackMode === 'individual' ? (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-[650px]">
                    <Card title={`Pending Reviews (${MOCK_STUDENTS.length})`} className="h-full flex flex-col">
                    ...
                    <Card className="h-full bg-gray-50/50 p-0" noPadding>
                        {isLoading ? (
                            ...
                            <div className="w-full h-full flex flex-col">
                                <div className="border-b border-gray-200 p-4 bg-white shadow-sm shrink-0 flex justify-between items-center z-10">
                                    <div>
                                    ...
                                    <div className="flex gap-2 relative">
                                        <Button variant="ghost" onClick={() => handleReview(selectedStudent)} className="!p-2 text-gray-400 hover:text-indigo-600" title="Regenerate">
                                            <RefreshCw size={18} />
                                        </Button>
                                        <div ref={feedbackShareRef} className="relative">
                                            <Button variant="secondary" onClick={() => setFeedbackShareOpen(!feedbackShareOpen)} className="!p-2 text-indigo-600" title="Export">
                                               <Share2 size={18} />
                                            </Button>
                                            {feedbackShareOpen && (
                                                <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-lg shadow-xl border border-gray-100 z-30 overflow-hidden animate-fade-in">
                                                    <button 
                                                        onClick={() => handleEmail(feedback, `Feedback for ${selectedStudent.name}`)}
                                                        className="w-full text-left px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-2"
                                                    >
                                                        <Mail size={16} /> Email
                                                    </button>
                                                    <button 
                                                        onClick={() => handlePrint(feedback, `Feedback: ${selectedStudent.name}`)}
                                                        className="w-full text-left px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-2"
                                                    >
                                ...
                                <div className="p-4 bg-gray-50 border-t border-gray-200 flex justify-end gap-3 shrink-0">
                                    <Button className="w-full justify-center">
                                       <Check size={16} /> Approve & Send Feedback
                                    </Button>
                                </div>
                            </div>
                        ) : (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <Card className="lg:col-span-2 min-h-[500px] bg-gray-50 flex flex-col p-0 overflow-hidden" noPadding>
                        {classFeedback ? (
                    ...
                    <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200 bg-white z-20 relative">
                        ...
                        <div className="flex gap-2 relative" ref={worksheetShareRef}>
                             ...
                             {isWorksheetShareOpen && (
                               <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-lg shadow-xl border border-gray-100 z-30 overflow-hidden animate-fade-in">
                                  <button 
                                    onClick={() => handleEmail(generatedWorksheet, `Worksheet: ${worksheetData.topic}`)}
                                    className="w-full text-left px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-2"
                                  >
                                    <Mail size={16} /> Email
                                  </button>
